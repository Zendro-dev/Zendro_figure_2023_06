...
module.exports = class country extends Sequelize.Model {
     ...
     */
    static async add_river_ids(country_id, river_ids, benignErrorReporter, token, handle_inverse = true) {
        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            river_ids.forEach(idx => {
                promises.push(models.river.add_country_ids(idx, [`${country_id}`], benignErrorReporter, token, false));
            });
     ...
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async remove_river_ids(country_id, river_ids, benignErrorReporter, token, handle_inverse = true) {
        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            river_ids.forEach(idx => {
                promises.push(models.river.remove_country_ids(idx, [`${country_id}`], benignErrorReporter, token, false));
            });
            await Promise.all(promises);
        }
